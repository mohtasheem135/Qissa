...
| shadcn primitives | [components/ui/](../components/ui/) | button, card, dialog, input, label, select, switch, table, textarea, badge, alert-dialog, sonner |
...
1. `GET /s/<id>/p/1` → [app/(public)/s/[storyId]/p/[partNumber]/page.tsx](../app/(public)/s/[storyId]/p/[partNumber]/page.tsx) fetches part + story + language
2. Per-language Google Fonts stylesheet is emitted via `<link>` from [lib/reader/google-fonts.ts](../lib/reader/google-fonts.ts)
3. [ReaderShell](../components/reader/ReaderShell.tsx) hydrates from `localStorage` (theme, font size, settings)
4. Service worker caches the HTML + any ImageKit thumb under that URL
5. Airplane mode → next visit: SW serves cached HTML; if not cached, falls back to `/offline`

### 3.3 Admin translates a 4-part story

1. Admin clicks **Translate 4 pending** on `/admin/stories/<id>`
2. [StoryEditShell](../components/admin/StoryEditShell.tsx) opens `fetch("/api/translate/queue")` with `{ storyId }`
3. [app/api/translate/queue/route.ts](../app/api/translate/queue/route.ts) returns a `text/event-stream` `ReadableStream`
4. Server iterates pending parts. For each: calls [lib/translation/run-part.ts](../lib/translation/run-part.ts) → `runStoryPartTranslation`
5. `runStoryPartTranslation` loads part + tone + language + previous-part context, marks part `translating`, runs [lib/ai/translate.ts](../lib/ai/translate.ts) → `withRetry(provider.translate(input))`, logs each attempt to `translation_jobs`, inserts a new `story_part_versions` row, updates `story_parts`
...
8. Cancellation: `AbortController` on the client → `request.signal.aborted` server-side stops the queue at the next part boundary
...
- **One core function**, [`runStoryPartTranslation`](../lib/translation/run-part.ts), is the source of truth for "translate one part end-to-end". Both endpoints delegate to it.
...
- **Failures** never throw out of the route. Part status becomes `failed` with `error_message`; admin can retry from the UI.
...
| Per-part reading progress | `qissa:progress:<storyId>:<partNumber>` | Cheap key-per-part; also drives ✓/⏳ indicators on story landing |
| Last-read pointer | `qissa:last-read` | Drives "Continue Reading" on home |
